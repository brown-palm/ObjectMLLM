...
def process_video_segment(video_path, start, end, processor, aspect_ratio='pad', num_frames=NUM_FRAMES, image_grid=False, sample_scheme='uniform'):
    def frame_sample(start, end, mode='uniform', local_fps=None):
        if mode == 'uniform':
            # Calculate the size of each segment from which a frame will be extracted
            duration = end - start
            seg_size = float(duration - 1) / num_frames
            ...
            for i in range(num_frames):
                # Calculate the start and end indices of each segment
                st = int(np.round(seg_size * i))
                ed = int(np.round(seg_size * (i + 1)))
                # Append the middle index of the segment to the list
                frame_ids.append(start + (st + ed) // 2)
    ...
    if isinstance(video_path, str):
        ...
        frame_id_list = frame_sample(start, end, mode=sample_scheme, local_fps=local_fps)

        assert len(frame_id_list) == num_frames
        try:
