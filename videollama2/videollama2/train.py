...
from videollama2.mm_utils import tokenizer_MMODAL_token, tokenizer_image_token, expand2square, process_video, process_image, process_video_segment
...
def preprocess_llama_2(
    ...
    # print('conv modality', conv.modality, '(train.py L278)')
        ...
        if 'image' in sources[0]:
            ...
            try:
                if 'start' in sources[0]:
                    # print('get a subvideo')
                    start = sources[0]['start']
                    end = sources[0]['end']
                    video = process_video_segment(video_file, start, end, video_processor, self.data_args.image_aspect_ratio, num_frames)
                else:
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # print('input_ids shape', input_ids.shape, '(in train.py L628)')
        if input_ids.shape[-2] > self.tokenizer.model_max_length:
            print('Warning: sequence too long!', input_ids.shape, self.tokenizer.model_max_length, '(train.py L631)')
