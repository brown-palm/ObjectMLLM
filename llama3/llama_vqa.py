import torch
import json
from llama import ModelArgs, Tokenizer_llama3, Transformer
...
def LLaMA_VQA(args, **kwargs):
    ...
    print(f"Using model: {args.llama_model_path}")
    ...
    for name, param in model_llama_vqa.named_parameters():
        if ('gate' in name) or ('adapter' in name) or ('temporal_emb' in name) or ('mm_projs' in name) or ('selector' in name) or ('query_tokens' in name):
