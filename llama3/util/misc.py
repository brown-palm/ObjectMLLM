...
def save_model(args, epoch, model, model_without_ddp, optimizer, loss_scaler, name):
    ...
    if loss_scaler is not None:
        ...
        for n, p in model_without_ddp.named_parameters():
            if ('gate' in n) or ('adapter' in n) or ('temporal_emb' in n) or ('visual_proj' in n) or ('mm_proj' in n) or ('query_token' in n):
                unfrozen_model[n] = p
...
def load_model(args, model_without_ddp, optimizer, loss_scaler):
    if args.resume:
        ...
        if args.box_format == 'textual_clip_yoloftsam':
            checkpoint['model'].pop('mm_projs.box.weight')
