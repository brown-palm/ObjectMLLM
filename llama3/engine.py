...
def val_one_epoch(model: torch.nn.Module, data_loader: Iterable, optimizer: torch.optim.Optimizer, epoch: int, use_vis, logits_dict=None, args=None):
    ...
    metric_logger = misc.MetricLogger(delimiter="  ")
    metric_logger.add_meter('lr', misc.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    metric_logger.add_meter('fnorm', misc.SmoothedValue(window_size=1, fmt='{value:.6f}'))
    header = 'Epoch: [{}]'.format(epoch)
    print_freq = 1
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with torch.no_grad():
            logits, d = model(data, use_vis=use_vis, inference=True)
        
        fnorm = d['fnorm'].detach().cpu().item() if d['fnorm'] is not None else 0.0
        ...
        metric_logger.update(fnorm=fnorm)
